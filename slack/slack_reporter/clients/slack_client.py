...
class SlackClient:
    ...
    def __init__(self) -> None:
        ...
        if not access_token:
            raise ValueError("The SLACK_BOT_TOKEN environment variable was not set. Cannot interact with Slack")
    ...
    def get_channel_id_by_name(self, channel_name: str) -> str | None:
        ...
        try:
            if channel_name:
                while True:
                    conversations = self.client.conversations_list(limit=100, cursor=next_cursor)
                    for channel in conversations["channels"]:
                        if channel["name"] == channel_name:
                            channel_id = channel["id"]
                            self.logger.info(f"Slack channel {channel_name} has ID {channel_id}")
                            return channel_id
    ...
    def send_message_in_thread_by_channel_id(self, channel_id: str, text: str, thread_ts: str = None) -> SlackResponse:
        ...
        """
        self.logger.info(f"Sending the message to thread {thread_ts} on channel {channel_id}")
        try:
            return self.client.chat_postMessage(channel=channel_id, mrkdwn=True, text=text, thread_ts=thread_ts,
                                                unfurl_links=False, unfurl_media=False)
        except SlackApiError as e:
            self.logger.error(f"Failed to send the Slack message: {e}")

    def add_reaction_to_thread_by_channel_id(self, channel_id: str, emoji_name: str = "approved", thread_ts: str = None) -> SlackResponse:
        """Send the given text to the given slack channel, thread.
        :param channel_id - id of the slack channel where you want to post your message)
        ...
        :param thread_ts - timestamp of the parent message (Optional - if None, then the message is posted as a parent message)
        """
        self.logger.info(f"Adding {emoji_name} reaction to thread {thread_ts}... on channel {channel_id} ")
        try:
            return self.client.reactions_add(channel=channel_id, name=emoji_name, timestamp=thread_ts)
        except SlackApiError as e:
            self.logger.error(f"Failed to add reaction to the Slack message: {e}")

    def send_message(self, channel_name: str = SlackConstants.SLACK_CHANNEL_NAME, text: str = None, thread_ts: str = SlackConstants.SLACK_THREAD_TS) -> SlackResponse:
        """
        ...
        return self.send_message_in_thread_by_channel_id(channel_id=self.channel_id, thread_ts=thread_ts, text=text)
